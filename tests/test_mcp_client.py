...
def settings() -> Settings:
    return Settings(host="192.168.1.50", api_key="a" * 32)
...
@pytest.mark.asyncio
async def test_health_sends_api_key_header(settings: Settings) -> None:
    seen = {}
...
@pytest.mark.asyncio
async def test_move_mouse_posts_body(settings: Settings) -> None:
    seen = {}
    ...
    client = make_client(settings, handler)
    result = await client.move_mouse(10, 20)
    assert result == {"status": "ok", "x": 10, "y": 20}
    assert seen["method"] == "POST"
    assert seen["path"] == "/mouse/move"
...
async def test_screenshot_returns_bytes_and_content_type(settings: Settings) -> None:
    ...
    client = make_client(settings, handler)
    data, content_type = await client.screenshot()
...
async def test_error_response_raises_journeycapture_error(settings: Settings) -> None:
    ...
    client = make_client(settings, handler)
    with pytest.raises(JourneyCaptureError, match="401"):
        await client.health()
