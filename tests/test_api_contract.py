...
def test_screenshot_monitors(client: TestClient, auth_headers: dict[str, str]) -> None:
    ...
    with patch("journeycapture_thinclient.routes.screenshot.capture.list_monitors", return_value=fake_monitors):
...
def test_screenshot(client: TestClient, auth_headers: dict[str, str]) -> None:
    with patch(
        "journeycapture_thinclient.routes.screenshot.capture.take_screenshot",
...
def test_mouse_move(client: TestClient, auth_headers: dict[str, str]) -> None:
    with patch("journeycapture_thinclient.routes.mouse.input_control.move_mouse", return_value=(10, 20)):
...
def test_mouse_scroll(client: TestClient, auth_headers: dict[str, str]) -> None:
    with patch("journeycapture_thinclient.routes.mouse.input_control.scroll_mouse") as mock_scroll:
...
def test_keyboard_type_logs_length_not_text(client: TestClient, auth_headers: dict[str, str], caplog) -> None:
    ...
    with caplog.at_level(logging.INFO, logger="journeycapture_thinclient.routes.keyboard"):
        with patch("journeycapture_thinclient.routes.keyboard.input_control.type_text", return_value=len(secret)):
...
def test_keyboard_key(client: TestClient, auth_headers: dict[str, str]) -> None:
    with patch("journeycapture_thinclient.routes.keyboard.input_control.send_keys") as mock_send:
...
def test_keyboard_key_invalid_key(client: TestClient, auth_headers: dict[str, str]) -> None:
    with patch(
        "journeycapture_thinclient.routes.keyboard.input_control.send_keys",
