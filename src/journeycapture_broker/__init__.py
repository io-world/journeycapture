...
async def _run(settings: Settings) -> None:
    ...
    uvicorn_config = uvicorn.Config(app, host=settings.host, port=settings.http_port, log_config=None)
    http_server = uvicorn.Server(uvicorn_config)

    logger.info(
        "Starting broker: HTTP on %s:%d, websocket on %s:%d, %d known machine(s)",
        settings.host,
        settings.http_port,
        settings.ws_host,
        settings.ws_port,
